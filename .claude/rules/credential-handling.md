...
Available functions:
- `load_splunk_credentials` — reads from project-root `credentials` or `~/.splunk/credentials`
- `get_session_key URI` — authenticates via stdin (password hidden from `ps`)
- `splunk_curl SK [args...]` — session key via process substitution
- `splunk_curl_post SK DATA [args...]` — POST data also via stdin
- `sanitize_response RESP` — strips secrets from API responses before display
