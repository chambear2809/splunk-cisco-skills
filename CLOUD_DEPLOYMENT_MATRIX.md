...
| `splunk-ai-ml-toolkit-setup` AI Toolkit | 2890 | ACS Splunkbase after installing the matching Python for Scientific Computing add-on | Search-tier validation plus UI handoffs for ML-SPL, Smart Assistants, anomaly workflows, LLM `ai` command providers, Connections, Container Management, ONNX, alerts, safeguards, and model inventory | Splunk AI Toolkit / MLTK (Splunkbase 2890). Latest audited release is 5.7.3 with Cisco Deep Time Series anomaly detection; live installs intentionally pull the latest compatible Splunkbase release and do not pin --app-version. |
| `splunk-ai-ml-toolkit-setup` PSC Linux 64-bit | 2882 | ACS Splunkbase for Linux search heads; default PSC target for Splunk Cloud | No app-local config; validates AI Toolkit Python runtime dependency | Python for Scientific Computing add-on for Linux 64-bit search heads, Splunkbase 2882, latest audited 4.3.1. |
| `splunk-ai-ml-toolkit-setup` PSC Windows 64-bit | 2883 | Splunkbase install on Windows search heads you control | No app-local config; validates AI Toolkit Python runtime dependency | Python for Scientific Computing add-on for Windows 64-bit search heads, Splunkbase 2883, latest audited 4.3.1. |
| `splunk-ai-ml-toolkit-setup` PSC Mac Intel | 2881 | Splunkbase install on Mac Intel search heads you control | No app-local config; validates AI Toolkit Python runtime dependency | Python for Scientific Computing add-on for Mac Intel search heads, Splunkbase 2881, latest audited 4.3.1. |
| `splunk-ai-ml-toolkit-setup` PSC Mac Apple Silicon | 6785 | Splunkbase install on Mac Apple Silicon search heads you control | No app-local config; validates AI Toolkit Python runtime dependency | Python for Scientific Computing add-on for Mac Apple Silicon search heads, Splunkbase 6785, latest audited 4.3.1. |
| `splunk-ai-ml-toolkit-setup` DSDL | 4607 | ACS Splunkbase after AI Toolkit and PSC prerequisites | Search-tier setup plus rendered handoffs for Docker, Kubernetes, OpenShift, HPC, GPU, air-gapped images, setup-page/runtime health, API endpoint, HEC/Observability telemetry, Jupyter notebooks, RAG/LLM, and model governance | Splunk App for Data Science and Deep Learning / DSDL (Splunkbase 4607). External runtimes remain operator-owned handoffs unless a downstream runtime skill applies them. |
| `splunk-db-connect-setup` | 2686 | ACS Splunkbase for supported Victoria search-tier deployments; Classic routes to IDM, customer-managed heavy forwarder, or Splunk Support | Render-first DBX settings, Java, driver, identity, connection, input, output, lookup, and validation handoffs; v1 does not mutate live DBX objects | Splunk DB Connect (Splunkbase 2686), audited at 4.2.4 on April 20, 2026. Requires Java 17 or 21 on Enterprise/customer-managed runtimes, Splunk-managed JRE validation on Cloud Victoria search heads, KV Store, JDBC drivers on every DBX runtime host, SHC deployer workflow for SHC, and explicit Cloud Victoria outbound database allowlist planning. |
| `splunk-db-connect-setup` Redshift JDBC add-on | 6149 | ACS Splunkbase wherever DB Connect is approved to run | Driver inventory and classpath validation only; DBX connection objects remain review/apply handoffs | Supported DBX JDBC add-on, Splunkbase 6149, latest audited 1.2.2. |
| `splunk-db-connect-setup` SQL Server JDBC add-on | 6150 | ACS Splunkbase wherever DB Connect is approved to run | Driver inventory plus SQL Server auth, stored procedure, Windows/Kerberos, and Entra planning handoffs | Supported DBX JDBC add-on, Splunkbase 6150, latest audited 1.3.2. |
| `splunk-db-connect-setup` Oracle JDBC add-on | 6151 | ACS Splunkbase wherever DB Connect is approved to run | Driver inventory plus wallet/client-certificate file handoffs | Supported DBX JDBC add-on, Splunkbase 6151, latest audited 2.2.2. |
| `splunk-db-connect-setup` PostgreSQL JDBC add-on | 6152 | ACS Splunkbase wherever DB Connect is approved to run | Driver inventory and classpath validation only; DBX connection objects remain review/apply handoffs | Supported DBX JDBC add-on, Splunkbase 6152, latest audited 1.2.2. |
| `splunk-db-connect-setup` Snowflake JDBC add-on | 6153 | ACS Splunkbase wherever DB Connect is approved to run | Driver inventory and classpath validation only; DBX connection objects remain review/apply handoffs | Supported DBX JDBC add-on, Splunkbase 6153, latest audited 1.2.4. |
| `splunk-db-connect-setup` MySQL JDBC add-on | 6154 | ACS Splunkbase wherever DB Connect is approved to run | Driver inventory and classpath validation only; DBX connection objects remain review/apply handoffs | Supported DBX JDBC add-on, Splunkbase 6154, latest audited 1.1.3. |
| `splunk-db-connect-setup` DB2 JDBC add-on | 6332 | ACS Splunkbase wherever DB Connect is approved to run | Driver inventory and classpath validation only; DBX connection objects remain review/apply handoffs | Supported DBX JDBC add-on, Splunkbase 6332, latest audited 1.1.1. |
| `splunk-db-connect-setup` MongoDB JDBC add-on | 7095 | ACS Splunkbase wherever DB Connect is approved to run | Driver inventory and classpath validation only; DBX connection objects remain review/apply handoffs | Supported DBX JDBC add-on, Splunkbase 7095, latest audited 1.3.0. |
| `splunk-db-connect-setup` Athena JDBC add-on | 8133 | ACS Splunkbase wherever DB Connect is approved to run | Driver inventory and classpath validation only; DBX connection objects remain review/apply handoffs | Supported DBX JDBC add-on, Splunkbase 8133, latest audited 1.0.1. |
...
| `splunk-data-source-readiness-doctor` | N/A | No Splunk Cloud app install; diagnoses whether onboarded data is usable by ES, ITSI, ARI, CIM, OCSF, and dashboards | Rendered readiness report, ES/ITSI/ARI scores, fix plan, collection SPL, registry projection, and handoff packets; live remediation remains delegated to product/setup skills | Post-ingest data-usability doctor. Consumes app registry hints, expected indexes/sourcetypes/macros, sample-event evidence, CIM tags/eventtypes, data-model acceleration state, OCSF transform status, and ES/ITSI/ARI readiness signals. |
| `splunk-spl2-pipeline-kit` | N/A | No Splunk Cloud app install; renders and lints SPL2 pipeline assets offline | SPL2 templates, custom pipeline template app stubs, compatibility lint, PCRE2 review, and profile checks for Ingest Processor and Edge Processor | Shared SPL2 authoring kit for Ingest Processor and Edge Processor. Covers route, redact, sample, metrics, OCSF, decrypt, stats, S3, custom templates, SPL-to-SPL2 review, and PCRE2 migration lint without live API calls. |
| `splunk-ingest-processor-setup` | N/A | No Splunk Cloud app install; Ingest Processor is a native Splunk Cloud Platform service provisioned through entitlement/support and UI workflows | Render-first readiness, source type, destination, SPL2 pipeline, lifecycle, monitoring, queue, Usage Summary, and downstream data-readiness handoffs; no private Ingest Processor API CRUD is claimed | Full Ingest Processor setup workflow for Splunk Cloud Victoria Experience. Uses splunk-spl2-pipeline-kit for SPL2, refuses Splunk Enterprise destinations in favor of Edge Processor, and hands off post-ingest content readiness to splunk-data-source-readiness-doctor. |
...
| `splunk-observability-aws-lambda-apm-setup` | N/A | No Splunk Cloud app install; the Splunk OTel Lambda layer (signalfx/splunk-otel-lambda, beta, publisher 254067382080) is attached directly to AWS Lambda functions via AWS CLI, CloudFormation, or Terraform | Rendered per-function layer ARN attachments, AWS_LAMBDA_EXEC_WRAPPER wiring, SPLUNK_REALM / SPLUNK_ACCESS_TOKEN (resolve references only — Secrets Manager or SSM SecureString), optional OTLP env vars, IAM egress stub (local collector disabled path), Terraform and CloudFormation snippets, and handoff scripts to splunk-observability-dashboard-builder and splunk-observability-native-ops | Render-first APM instrumentation skill for AWS Lambda via the Splunk OpenTelemetry Lambda layer. Covers Node.js 18/20/22, Python 3.9-3.13, Java 8/11/17/21 on x86_64 and arm64. Per-runtime AWS_LAMBDA_EXEC_WRAPPER wiring; secret-safe SPLUNK_ACCESS_TOKEN delivery via AWS Secrets Manager or SSM SecureString resolve references; layer ARN baked snapshot with opt-in live refresh; vendor/ADOT conflict detection; X-Ray coexistence; GovCloud/China refusal; beta gate. Fulfills the handoffs.lambda_apm stub in splunk-observability-aws-integration. |
...
| `splunk-observability-deep-native-workflows` | N/A | No Splunk Cloud app install; renders UI-aware native Observability Cloud workflow packets and API-vs-UI coverage reports | Render modern dashboard, APM service map/service view/business transaction/trace/profiling, RUM replay/error/URL grouping/mobile, DBMon, Synthetics, SLO, Infrastructure/Kubernetes/Network Explorer, Related Content, AI Assistant, logs chart, and Observability Cloud Mobile app plans with API-vs-UI status, deeplinks, API action plans, and downstream skill handoffs | Deep native Observability workflow planner for product surfaces that need UI-aware runbooks or public API action plans. Complements dashboard-builder, native-ops, cloud-integration, RUM, DBMon, and OTel collector skills without owning collector runtime placement. |
