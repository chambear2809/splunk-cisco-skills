...
| `galileo-platform-setup` | None | None | None | None | Required | External collector | Galileo SaaS/Enterprise readiness and Splunk workflow orchestration: export_records to Splunk HEC, Observe OpenTelemetry/OpenInference snippets, Protect invoke snippets, Evaluate/experiment/dataset/metric/annotation handoffs, Splunk Connect for OTLP handoff, Splunk OTel Collector handoff, and dashboard/detector handoffs. Use --o11y-only for Splunk Observability Cloud-only mode with no Splunk Platform HEC dependency. |
