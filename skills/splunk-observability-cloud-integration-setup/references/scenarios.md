...
- The skill renders and validates the full plan tree without live mutation.
- Apply supported sections explicitly after review. Centralized RBAC cutover
  and Related Content role-capability changes remain fail-closed handoffs.
...
# Render the reviewed per-org handoff. Live apply fails before mutation because
# the current CLI cannot safely map one distinct token file to each org.
bash skills/splunk-observability-cloud-integration-setup/scripts/setup.sh \
  --render \
...
```bash
bash skills/splunk-observability-cloud-integration-setup/scripts/setup.sh \
  --apply pairing,discover_app,sim_addon \
  --spec spec.yaml \
...
## 4. Migrate API-Token Pairing -> Unified Identity

An existing customer that has used API-token pairing wants to move to Unified
Identity. The skill detects the existing token connection, renders a numbered
migration plan, and produces deeplinks for the old connection's
...
After the operator confirms UID is healthy, use the approved Splunk Cloud
administrative workflow for destructive RBAC cutover; this repo fails closed.
...
Discover-app pairing and Related Content previews are also `not_applicable`.
The skill can configure the SIM Add-on and the Splunk Enterprise Log Observer
Connect service-account/TLS-certificate path.
...
a Splunk Observability Cloud integration. They render an inventory scaffold,
run the limited live reachability checks, then review the static doctor matrix
before targeted apply.

```bash
# Write the rendered-plan inventory scaffold (no live snapshot claim).
bash skills/splunk-observability-cloud-integration-setup/scripts/setup.sh \
  --discover \
  --realm us0

# Read-only token-auth and SIM-account endpoint reachability.
bash skills/splunk-observability-cloud-integration-setup/scripts/setup.sh \
  --validate --live \
  --realm us0
...
The `--discover` mode is read-only and writes an honest inventory scaffold;
it does not claim full live discovery. The static `--doctor` matrix records
fix commands or explicit handoffs, depending on whether a safe apply path exists.
