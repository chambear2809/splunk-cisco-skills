...
def render_apply_script(spec: dict[str, Any], workloads: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "        python3 - \"${safekey}\" \"${backup_file}\" <<'PY' | \\\n"
        "            kubectl -n \"${BACKUP_NAMESPACE}\" patch configmap splunk-rum-injection-backup --type=merge --patch-file=/dev/stdin\n"
        "import json, sys\n"
        "print(json.dumps({'data': {sys.argv[1]: open(sys.argv[2], encoding='utf-8').read()}}))\n"
        "PY\n"
        "    else\n"
        "        echo \"ERROR: could not snapshot ${kind}/${name}; refusing to mutate without a backup\" >&2\n"
        "        rm -f \"${backup_file}\"\n"
        "        exit 1\n"
        "    fi\n"
        "    rm -f \"${backup_file}\"\n"
...
def render_uninstall_script(spec: dict[str, Any], workloads: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "                    splunk.com/rum-target-workload-\n"
        "                ;;\n"
        "        esac\n"
        "    done\n"
        ...
        "    fi\n"
        "done\n"
        "kubectl -n default delete --ignore-not-found configmap splunk-rum-injection-backup\n"
    )
...
def render_verify_script(spec: dict[str, Any], workloads: list[dict[str, Any]]) -> str:
    return (
        ...
        "    if [[ -z \"${pod}\" ]]; then\n"
        "        echo \"FAIL: no pod found for ${target}\" >&2\n"
        "        failures=$((failures + 1))\n"
        "        continue\n"
        "    fi\n"
        "    port=8080\n"
...
def render_sourcemap_upload_script(spec: dict[str, Any]) -> str:
    ...
    target_dir = str(sm.get("injection_target_dir") or "dist")
    if "\n" in target_dir or "\r" in target_dir or "\x00" in target_dir:
        raise SpecError("source_maps.injection_target_dir must be a single-line path")
    ...
    return (
        ...
        f"[[ -n \"${{TARGET_DIR}}\" ]] || TARGET_DIR={target_dir_q}\n"
