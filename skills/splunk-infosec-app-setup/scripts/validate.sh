...
cmd=(bash "${REPO_ROOT}/skills/shared/scripts/validate_render_first_skill.sh" --skill-name splunk-infosec-app-setup --profile-dir splunk-infosec-app --default-output splunk-infosec-app-rendered --app-name infosec_app_for_splunk --require-dashboard)
...
if [[ "${LIVE}" == "true" ]]; then
    ...
    rest_check_app "${SK}" "${SPLUNK_URI}" infosec_app_for_splunk || { echo "ERROR: infosec_app_for_splunk is not installed." >&2; exit 1; }
fi
