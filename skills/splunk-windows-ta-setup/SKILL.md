...
description: "Use when the user asks about Splunk_TA_windows, the Splunk Add-on for Microsoft Windows, WinEventLog or
  Perfmon inputs, Windows Security event onboarding, Sysmon, or Windows CIM readiness in Splunk. Install,
  render, configure, and validate the Splunk Add-on for Microsoft Windows (Splunk_TA_windows, Splunkbase
...
```bash
bash skills/splunk-windows-ta-setup/scripts/validate.sh --help
```
