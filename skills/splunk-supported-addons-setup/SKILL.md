...
description: >-
  ...
  Linux add-on setup, Splunk_TA_nix, Splunk_TA_Linux, Linux CollectD, auditd,
  *nix scripted inputs, or router guidance before using splunk-app-install.
---

# Splunk Supported Add-ons Setup

Use this skill as the router for Splunk-supported add-ons before choosing an
install or configuration workflow. The first implemented domain profile is
Unix/Linux, covering both:
...
- Renders a reviewable setup plan, starter input overlays, validation SPL, and
  handoff commands.
- For official add-ons outside the first-class Unix/Linux domain, renders a
  generic handoff packet with official docs and local, remote, or Splunkbase
  package install templates.
...
- Do not install directly from this router. Use the emitted
  `splunk-app-install` command for package delivery.
