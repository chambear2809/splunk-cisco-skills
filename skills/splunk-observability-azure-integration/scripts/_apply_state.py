"""Shared apply-state.json helpers for the Splunk Observability Azure integration API client.
...
contain a token, password, Azure application secret, or API key (the redactor
strips those).
...
def is_sensitive_key(key: Any) -> bool:
    ...
    return canonical.endswith("key") or "credential" in canonical or any(s in canonical for s in (
        ...
        "accesskey", "awsaccesskey", "secretkey", "projectkey", "wifconfig",
    ...
    *,
    ...
    if _directory is not None:
        _write_private_json_at(
            _directory, path, value, redact_value=redact_value
        )
        return
    with secure_private_directory(
...
def _load_state(
    state_path: Path, directory: SecureDirectory
) -> dict[str, Any]:
    try:
        metadata = os.stat(
            state_path.name, dir_fd=directory.fd, follow_symlinks=False
        )
    except FileNotFoundError:
        return {"steps": []}
    current_uid = os.getuid() if hasattr(os, "getuid") else metadata.st_uid
