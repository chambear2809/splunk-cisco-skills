...
This workflow imports and configures content packs in an **existing** ITSI
deployment. It does not install ITSI, a legacy Splunk App for Content Packs, or
any TA/SA/DA/dashboard prerequisite. The compatible Content Library API must
already be available through a provider documented for the detected ITSI
version. Splunk App for Content Packs 2.5 explicitly lists ITSI 4.20.x and
4.21.x; do not infer 5.0 compatibility or built-in legacy discovery routes from
the modernized 5.0 Content Library interface alone. Missing prerequisites
produce an explicit handoff to `splunk-itsi-setup`, `splunk-app-install`, or the
Splunk Cloud app-request process.

Copy `templates/beginner.content-pack.yaml` to the gitignored
`skills/splunk-itsi-config/template.local`, and keep one pack in the first run.
Use `references/beginner_quickstart.md` to map the product/domain to a profile,
exact catalog title or ID, and required index or macro values.

Run lint, GET-only preview, explicit apply, and completion validation from the
repository root:
...
# Only after explicit approval of the preview:
bash skills/splunk-itsi-config/scripts/setup.sh \
  --workflow content-packs \
  --spec skills/splunk-itsi-config/template.local \
  --apply
...
Preview and validate read the existing catalog without refreshing it. Catalog
discovery and refresh are writes. When a stale catalog requires a refresh, set
`content_library.refresh_catalog: true` only for a separately approved apply.
The applicable product endpoints include:
...
Preview and content-pack import use the matching route family with the same
suffixes. Splunk names the import endpoint `/install`; this does not authorize
installation of ITSI or prerequisite app packages:
...
The following is an advanced field catalog, not a copy-and-apply starter. It
contains several packs and enabled dispatch/import/upload guards for illustration.
Build a minimal `template.local` with one pack and leave every guarded outcome
false until its exact target and effect are separately approved.
...
Profile names fall into two groups:

- Rich local readiness checks: `aws`, `cisco_data_center`,
  `cisco_enterprise_networks`, `cisco_thousandeyes`, `linux`,
  `splunk_appdynamics`, `splunk_observability_cloud`, `vmware`, and `windows`.
- Catalog-generic resolution/import with explicit module handoffs: the remaining
  named profiles and exact live catalog-title/ID entries.

A named profile does not imply end-to-end dashboard/data validation. See
`product_coverage.md` and the per-profile notes below.

## Workflow Notes

- ITSI and a compatible Content Library API are hard prerequisites. This workflow never
  bootstraps app 1841 or 5391, extracts app bundles, uses SSH for package
  placement, or restarts Splunk.
- Missing ITSI hands off to `splunk-itsi-setup`. A missing version-compatible
  legacy Content Library app or pack prerequisite hands off to
  `splunk-app-install` or Splunk Cloud Support. Do not install app 5391 merely
  because a newer ITSI release does not expose that legacy app name; first check
  that release's documented Content Library packaging and live catalog API.
- The workflow checks the bundled ITSI app set (`SA-ITOA`, `itsi`,
  `SA-UserAccess`, `SA-ITSI-Licensechecker`) plus KV Store readiness and key
  ITSI collections before content configuration.
...
- Preview, apply, and validate do not install prerequisite apps. A missing app
  is never fixed by rerunning this configuration workflow with `--apply`.
