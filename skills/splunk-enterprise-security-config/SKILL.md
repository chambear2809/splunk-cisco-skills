...
description: "Use when the user asks to configure, tune, validate, or operationalize Splunk Enterprise Security.
  Configure and validate Splunk Enterprise Security (ES) after installation, including ES indexes,
  Splunk_TA_ForIndexers readiness, users and roles, managed custom roles, CIM data models, data model
  acceleration, assets and identities, threat intelligence, detections, risk-based alerting, Mission
  Control, exposure analytics, UEBA, native SOAR integrations, ES AI Assistant settings, Federated
  Analytics inventory/handoffs, and configuration health checks."
...
- Configure, tune, validate, or operationalize Splunk Enterprise Security.
- Preview and review the splunk enterprise security config workflow before any live apply phase.
...
```bash
bash skills/splunk-enterprise-security-config/scripts/setup.sh --help
```
...
```bash
bash skills/splunk-enterprise-security-config/scripts/validate.sh --help
```
