...
def _blocks_ssl_certificate_checker(args: argparse.Namespace) -> bool:
    """Return true when Splunkbase 3172 does not list the target train."""
    return _splunk_version_tuple(args.splunk_version) >= (10, 0, 0)
...
def render_readme(out: Path, args: argparse.Namespace, targets: set[str], mtls: set[str]) -> None:
    certificate_monitoring = (
        "Splunkbase 3172 is blocked for this target because its listing stops at 9.4. Use the rendered "
        "`pki/rotate/expire-watch.sh` and `inventory.sh` helpers."
        if _blocks_ssl_certificate_checker(args)
        else "Use the rendered native expiry helpers by default; Splunkbase 3172 is an optional legacy handoff only after checking its listed target versions."
    )
...
# THRESHOLD days. Wraps `openssl x509 -enddate` and is the native monitoring
# path for current 10.x runtimes, where SSL Certificate Checker (Splunkbase
# 3172) must not be installed. Splunk Cloud 10.5 is not a self-managed runtime.
...
def _handoff_operator_checklist(args: argparse.Namespace, targets: set[str], mtls: set[str]) -> str:
    ...
    certificate_monitoring_check = (
        "- [ ] Splunkbase 3172 was not installed; native `pki/rotate/expire-watch.sh` and `inventory.sh` monitoring is scheduled."
        if _blocks_ssl_certificate_checker(args)
        else "- [ ] Native expiry monitoring is scheduled; optional Splunkbase 3172 was considered only after checking its listed target versions."
    )
...
def _handoff_post_install_monitoring(args: argparse.Namespace) -> str:
    certificate_monitoring_check = (
        "- [ ] **Target-version block:** do not install SSL Certificate Checker (Splunkbase 3172); its listing stops at 9.4."
        if _blocks_ssl_certificate_checker(args)
        else "- [ ] Treat SSL Certificate Checker (Splunkbase 3172) as optional legacy coverage only after verifying the target version on Splunkbase."
    )
    return f"""# Handoff — Post-Install Monitoring

See references/post-install-monitoring.md for the full set. Quick
checklist:

{certificate_monitoring_check}
- [ ] Native `pki/rotate/expire-watch.sh` scheduled at 30 / 14 / 7-day thresholds.
- [ ] Native `inventory.sh` PEM expiry output collected from every managed host.
