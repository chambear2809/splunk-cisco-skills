...
| KV Store backup | `--backup-kvstore` | Apply via SSH profile | Runs `splunk backup kvstore -archiveName es_kvstore_<timestamp>` via deployer/local profile; fails with a non-executed manual command when no profile is configured |
