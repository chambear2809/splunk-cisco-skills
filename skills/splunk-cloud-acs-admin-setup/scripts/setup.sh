...
Examples:
  ...
  $(basename "$0") --target-stack example-stack --phase render --admin-plan-file acs-admin-plan.json
  $(basename "$0") --target-stack example-stack --phase audit
  ACCEPT_ACS_ADMIN_MUTATION=true $(basename "$0") --target-stack example-stack --phase apply --admin-plan-file acs-admin-plan.json
