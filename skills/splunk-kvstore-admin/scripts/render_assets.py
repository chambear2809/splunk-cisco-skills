...
BUNDLE_OWNER = "splunk-kvstore-admin"
...
if [[ -z "${{runtime_home}}" && -n "${{splunk:-}}" ]]; then
  runtime_home="$(cd "$(dirname "${{splunk}}")/.." && pwd)"
fi
[[ -n "${{runtime_home}}" ]] || {{ echo "ERROR: rendered script did not set splunk or splunk_home." >&2; exit 1; }}
