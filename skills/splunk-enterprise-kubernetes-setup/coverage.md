...
| Existing same-namespace SOK LicenseManager | First-class | Name and an omitted or explicitly matching namespace for an Operator-managed v4 LicenseManager CR. Fresh preflight requires exact API identity, a separate currently deployed Helm owner release, complete UID/resourceVersion/generation, no terminating/paused/admin-managed-PV state, and a current clean `Ready` status. This flag is not an arbitrary external Splunk endpoint. |
...
| New install | First-class | Preflight accepts an absent or healthy pre-staged namespace without adopting an existing deployment; exact global CRD/CR inventory gates orphaned-CRD reuse before mutation. The path then creates only absent namespaces, applies the same hash-verified staged CRD bytes used by preflight, runs combined server dry-run, and installs Operator, optional license, and Enterprise before exact status checks. CRDs must exist before Enterprise resources can be dry-run. |
