...
usage() {
    cat >&2 <<EOF
Usage: $(basename "$0") [--product esa|wsa|both] [--esa-index INDEX] [--wsa-index INDEX] [--strict|--completion]

Diagnostic mode reports incomplete onboarding as warnings. --strict and its
alias --completion make completion-critical findings exit nonzero. These parser
TAs ship no dashboards; completion is based on macro and event evidence.
EOF
    exit "${1:-0}"
}
...
check_product() {
    ...
    if platform_check_index "${SK}" "${SPLUNK_URI}" "${index}"; then
        ...
        completion_issue "Index ${index} not found"
    fi
    if ! ${app_present}; then
    ...
    if [[ -n "${def}" && "${def}" == *"${index}"* ]]; then
        pass "Macro ${macro} includes ${index} (${def})"
    elif [[ -n "${def}" ]]; then
        completion_issue "Macro ${macro} does not include selected index ${index} (${def})"
    else
    ...
    if [[ "${count}" -gt 0 ]]; then
        ...
        completion_issue "No recent/default-index data found for ${app}"
    fi
}
