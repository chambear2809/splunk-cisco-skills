...
for product in "${selected_products[@]}"; do case "${product}" in epm|epv_pta) ;; *) echo "ERROR: Unsupported CyberArk product: ${product}" >&2; exit 1 ;; esac; done
...
install_packages(){ local restart=(); [[ "${NO_RESTART}" == "true" ]] && restart+=(--no-restart); contains "${PRODUCTS}" "epm" && run_or_print bash "${APP_INSTALL_SCRIPT}" --source splunkbase --app-id 5160 --no-update "${restart[@]}"; contains "${PRODUCTS}" "epv_pta" && run_or_print bash "${APP_INSTALL_SCRIPT}" --source splunkbase --app-id 2891 --no-update "${restart[@]}"; return 0; }
