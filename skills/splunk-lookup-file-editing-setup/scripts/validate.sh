...
cmd=(bash "${REPO_ROOT}/skills/shared/scripts/validate_render_first_skill.sh" --skill-name splunk-lookup-file-editing-setup --profile-dir splunk-lookup-file-editing --default-output splunk-lookup-file-editing-rendered --app-name lookup_editor --require-dashboard)
...
if [[ "${LIVE}" == "true" ]]; then
    ...
    rest_check_app "${SK}" "${SPLUNK_URI}" lookup_editor || { echo "ERROR: lookup_editor is not installed." >&2; exit 1; }
fi
