...
DEFAULT_OUTPUT_DIR="${PROJECT_ROOT}/splunk-galileo-rendered"
RENDERER="${SCRIPT_DIR}/render_assets.py"
VALIDATE_SCRIPT="${SCRIPT_DIR}/validate.sh"

APPLY_SECTIONS_DEFAULT="hec-service,hec-export,otlp-input,otel-collector,python-runtime,kubernetes-runtime,dashboards,detectors"

usage() {
    cat <<'EOF'
Splunk Galileo Integration setup

Usage:
  bash skills/splunk-galileo-integration/scripts/setup.sh [mode] [options]
...
Modes:
  ...
  --apply [SECTIONS]            Render then apply selected comma-separated sections.
                                ...
                                Observability Cloud sections with --o11y-only.
  ...
  --o11y-only                   Pure Splunk Observability Cloud mode. Skips
                                Splunk Platform HEC/OTLP defaults and the
                                OTel Collector platform HEC helper.

Apply sections:
  hec-service                   Delegate HEC token/service setup to splunk-hec-service-setup
  hec-export                    Run the Galileo REST export to Splunk HEC bridge
  otlp-input                    Delegate Splunk Platform OTLP input to splunk-connect-for-otlp-setup
  otel-collector                Delegate Splunk OTel Collector setup to splunk-observability-otel-collector-setup
  python-runtime                Copy or point to Python OpenTelemetry/OpenInference snippets
  kubernetes-runtime            Render/apply Kubernetes runtime env and annotation helpers
...
Configuration:
  --spec PATH                   Optional YAML/JSON spec (api_version: splunk-galileo-integration/v1)
...
apply_section() {
    ...
    case "${section}" in
        hec-service) script="apply-hec-service.sh" ;;
        hec-export) script="apply-hec-export.sh" ;;
        otlp-input) script="apply-otlp-input.sh" ;;
        otel-collector) script="apply-otel-collector.sh" ;;
        python-runtime) script="apply-python-runtime.sh" ;;
        kubernetes-runtime) script="apply-kubernetes-runtime.sh" ;;
...
if [[ "${MODE_DOCTOR}" == "true" ]]; then
    log "Galileo integration doctor completed. Review ${OUTPUT_DIR}/coverage-report.json and ${OUTPUT_DIR}/handoff.md."
fi
...
if [[ "${MODE_APPLY}" == "true" ]]; then
    ...
    if [[ "${O11Y_ONLY}" == "true" && ( -z "${APPLY_SECTIONS}" || "${APPLY_SECTIONS}" == "all" ) ]]; then
        sections="otel-collector,python-runtime,kubernetes-runtime,dashboards,detectors"
    fi
    ...
    for section in "${section_array[@]}"; do
        ...
        if [[ "${O11Y_ONLY}" == "true" ]]; then
            case "${section}" in
                hec-service|hec-export|otlp-input)
