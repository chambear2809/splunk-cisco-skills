...
def parse_args() -> argparse.Namespace:
    reject_direct_secret_flags(sys.argv[1:])
    parser = argparse.ArgumentParser(
        description="Pull Galileo Observe records and send them to Splunk HEC."
    )
    ...
    parser.add_argument("--root-type", choices=sorted(ROOT_ENDPOINTS), default=os.getenv("GALILEO_ROOT_TYPE", "trace"))
    ...
    parser.add_argument("--filter-key", choices=["name", "column_id"], default="name")
...
def query_galileo(args: argparse.Namespace, since: str | None) -> list[dict[str, Any]]:
    endpoint = ROOT_ENDPOINTS[args.root_type]
    url = f"{args.galileo_api_base.rstrip('/')}/v2/projects/{args.project_id}/{endpoint}/search"
    ...
    while True:
        body: dict[str, Any] = {
            ...
            "limit": args.limit,
            "sort": {
                args.filter_key: args.sort_field,
                "ascending": True,
                "sort_type": "column",
            },
        ...
        payload = request_json("POST", url, headers, body)
        page_records = payload.get("records", [])
        if not isinstance(page_records, list):
            raise RuntimeError(f"Unexpected Galileo response: records is {type(page_records).__name__}")
        ...
        if next_token is None or next_token == starting_token or not page_records:
            return records
