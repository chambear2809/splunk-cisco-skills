...
| `splunk-observability-gcp-integration` | GCP -> Splunk Observability Cloud | Standalone reusable: render, apply, validate, discover, and diagnose the Splunk Observability Cloud GCP integration end-to-end. REST `type=GCP`; two `authMethod` values: `SERVICE_ACCOUNT_KEY` (SA JSON key file, chmod 600) and `WORKLOAD_IDENTITY_FEDERATION` (pool + provider + Splunk principal); `projectKey` is write-only and redacted on GET — drift detection uses SHA-256 hash sidecar; 32-entry services enum; `customMetricTypeDomains`; `excludeGceInstancesWithLabels`; `useMetricSourceProjectForQuota` (warns about `serviceusage.serviceUsageConsumer`); `pollRate` 60–600 s (wire milliseconds); `namedToken` ForceNew warning; Terraform `signalfx_gcp_integration` pinned to `splunk-terraform/signalfx ~> 9.0`; `gcloud` SA creation + key + IAM role-binding scripts; WIF realm-to-principal map (all 10 realms); conflict matrix (SA Key vs WIF mutual exclusion, `services.explicit` vs `all_built_in`, quota role prerequisite); cross-skill handoffs to `splunk-app-install` (Splunkbase 3088 GCP logs), `splunk-observability-otel-collector-setup` (GKE host telemetry), `splunk-observability-dashboard-builder`, and `splunk-observability-native-ops`. |
