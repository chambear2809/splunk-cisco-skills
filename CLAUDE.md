...
| `splunk-appdynamics-dual-agent-setup` | AppDynamics Java Dual Signal | Render, preflight, apply, validate, and rollback production Java Dual Signal startup configuration after collector-first validation; gates host mutation, SSH execution, app restarts, and full restart strategy |
...
| `splunk-appdynamics-machine-agent-otel-collector-setup` | AppDynamics Machine Agent bundled OTel Collector | Render, preflight, apply, validate, and rollback Machine Agent bundled collector configuration for RPM, ZIP, Docker, and Windows ZIP layouts with loopback OTLP defaults and file-backed secrets |
...
| `cisco-asa-ta-setup` | `Splunk_TA_cisco-asa` | Render Cisco ASA and FTD syslog onboarding, cisco:asa source typing, SC4S/syslog ownership, CIM readiness, and validation evidence |
...
| `widefield-security-setup` | WideField Security adoption router | Render source-backed WideField adoption plans and delegate Okta, Saviynt, Splunk SIEM, Google SecOps, and identity-threat doctor workflows without undocumented WideField APIs |
| `widefield-okta-integration-setup` | WideField Security + Okta | Render OIN/shared-signal handoffs and safely apply documented Okta event hook create/update/verify/deactivate actions with file-backed credentials |
| `widefield-saviynt-integration-setup` | WideField Security + Saviynt | Render Saviynt remediation mappings for revoke access, password reset, and micro-certification; live Saviynt mutation fails closed until official/customer API docs are added |
| `widefield-splunk-siem-setup` | WideField Security -> Splunk SIEM | Create the `widefield` index, constrained HEC handoff, schema-light searches, saved search, macro, and starter dashboard for WideField events |
| `widefield-google-secops-setup` | WideField Security -> Google SecOps | Render feed/webhook/parser assets and validate evidence for Google SecOps log type `WIDEFIELD_SECURITY`; live feed creation is disabled without documented API coverage |
| `widefield-identity-threat-doctor` | WideField identity threat coverage | Run read-only checks for OAuth abuse, connected-app permission risk, NHI ownership, MFA posture, AI-agent identities, and session anomalies; render gated remediation packets |
...
| `splunk-security-content-update-setup` | `DA-ESS-ContentUpdate` | Render Enterprise Security Content Update install/upgrade plans, analytic-story inventory checks, and correlation-search activation review |
| `splunk-lookup-file-editing-setup` | Splunk App for Lookup File Editing | Render Lookup File Editing install readiness, CSV/KV Store lookup inventory checks, SHC backup replication notes, and ownership handoffs |
| `splunk-infosec-app-setup` | InfoSec App for Splunk | Render InfoSec App install readiness, prerequisite security data-source checklists, dashboard/macro checks, and CIM handoffs |
| `splunk-pci-compliance-setup` | Splunk App for PCI Compliance | Render PCI Compliance installer selection, CDE index/macro intake, CIM prerequisites, roles, reports, and dashboard evidence |
| `splunk-fraud-analytics-setup` | Splunk App for Fraud Analytics | Render Fraud Analytics prerequisites, fraud use-case intake, ES/RBA dependencies, correlation-search review, and validation SPL |
...
| `splunk-supported-addons-setup` | Splunk Supported Add-ons router, first profile Unix/Linux | Track official supported-addons coverage and resolve entries to first-class Unix/Linux plans, skill handoffs, or generic install-only paths; covers `Splunk_TA_nix` and `Splunk_TA_Linux` |
| `splunk-windows-ta-setup` | `Splunk_TA_windows` | Render reviewable WinEventLog/Perfmon/WinHostMon `inputs.conf` overlays, create the wineventlog/perfmon indexes, enforce UF/HF/search-tier placement, map source types to CIM, and validate Windows data; forwarder rollout via `splunk-agent-management-setup` |
| `splunk-microsoft-cloud-setup` | `splunk_ta_o365` (4055) + `Splunk_TA_microsoft-cloudservices` (3110) | Render Office 365 Management Activity (incl. Entra/Azure AD), Microsoft Graph Entra ID metadata, service status, and Azure audit inputs; emit an Entra app-registration account runbook; create the o365/azure indexes; validate ingestion |
| `splunk-aws-ta-setup` | `Splunk_TA_aws` | Manual TA path (complements Data Manager) rendering CloudTrail and GuardDuty via `aws_sqs_based_s3` and AWS Config via `aws_config`, an IAM-role/access-key account runbook, the aws index, CIM mapping, and ingestion validation |
| `splunk-amazon-kinesis-firehose-setup` | Amazon Kinesis Firehose to Splunk HEC | Render Firehose-to-HEC destination settings, IAM/S3 backup stubs, HEC handoffs, delivery metrics, and source/sourcetype readiness evidence |
| `splunk-okta-ta-setup` | `Splunk_TA_okta_identity_cloud` | Render `okta_identity_cloud` inputs across the log/user/group/app metrics (`OktaIM2:*` source types), an OAuth 2.0 or API-token account runbook, the okta index, CIM Authentication mapping, and ingestion validation |
| `splunk-gcp-ta-setup` | `Splunk_TA_google-cloudplatform` | Render the Cloud Logging to Pub/Sub log path (`google_cloud_pubsub` -> `google:gcp:pubsub:message` plus audit subtypes), a service-account/Pub/Sub-export runbook, the gcp index, and ingestion validation (metrics path is `splunk-observability-gcp-integration`) |
| `splunk-servicenow-ta-setup` | `Splunk_TA_snow` | Render per-table `snow://` inputs (incident, change_request, problem, em_event, CMDB) with account/timefield/id_field, a basic-auth or OAuth account runbook, the snow index, `snow:<table>` source types, and ingestion validation |
| `splunk-google-workspace-ta-setup` | `Splunk_TA_Google_Workspace` | Render Google Workspace Admin Reports, Gmail logs, identity, Alert Center, and usage-report inputs, service-account certificate runbooks, the google_workspace index, and ingestion validation |
| `splunk-microsoft-security-ta-setup` | `Splunk_TA_MS_Security` | Render Microsoft Defender incidents, endpoint alerts, machines, simulations, Event Hub, and threat-intelligence inputs, Entra app account runbooks, package macros, the microsoft_security index, and ingestion validation |
| `splunk-microsoft-exchange-ta-setup` | Microsoft Exchange Supported Add-ons | Render, install, and validate Microsoft Exchange add-on bundle and Exchange Indexes package readiness, including Windows collection placement, msexchange/perfmon/windows indexes, and readiness-doctor handoffs |
| `splunk-microsoft-scom-ta-setup` | `Splunk_TA_microsoft-scom` | Render, install, and validate Microsoft SCOM add-on readiness, including package-derived PowerShell inputs, `microsoft:scom*` source types, eventtypes, lookups, and readiness-doctor handoffs |
| `splunk-sysmon-ta-setup` | `Splunk_TA_microsoft_sysmon` | Render endpoint or Windows Event Collector Sysmon WinEventLog overlays, prevent duplicate direct-plus-WEC ingestion, hand off UF rollout, create the sysmon index, and validate source-constrained Sysmon data |
| `splunk-github-ta-setup` | `Splunk_TA_github` | Render GitHub Cloud audit, user, code scanning, Dependabot, and secret scanning alert inputs plus PAT/HEC/GHES handoffs, the github index, and ingestion validation |
| `splunk-salesforce-ta-setup` | Splunk_TA_salesforce | Render Salesforce object and event-log inputs, account runbooks, the salesforce index, package-backed sfdc source types, and readiness validation |
| `splunk-box-ta-setup` | Splunk_TA_box | Render Box historical event, live monitoring, and file-ingestion inputs, OAuth account handoffs, the box index, and readiness validation |
| `splunk-cyberark-ta-setup` | Splunk_TA_cyberark_epm + Splunk_TA_cyberark | Render supported CyberArk EPM API inputs plus archived/not-supported EPV/PTA CEF parser and SC4S/syslog handoffs |
| `splunk-rsa-securid-ta-setup` | Splunk_TA_rsa_securid_cas + Splunk_TA_rsa-securid | Render RSA SecurID CAS Cloud Administration API inputs plus Authentication Manager syslog/parser handoffs |
| `splunk-security-appliance-ta-setup` | Security Appliance Supported Add-ons | Render, install, and validate package-verified Carbon Black and Symantec Endpoint Protection add-ons, including file/syslog transport ownership, package-derived source types, eventtypes, lookups, and readiness-doctor handoffs |
| `splunk-syslog-web-proxy-ta-setup` | Web/proxy/parser Supported Add-ons | Render Apache, NGINX, IIS, Tomcat, HAProxy, Squid, Blue Coat, Forcepoint, Check Point, F5, Citrix, and Infoblox transport profiles |
| `splunk-vmware-ta-setup` | VMware Supported Add-ons | Render VMware app/add-on placement, vCenter collection runbooks, ESXi syslog handoffs, event and metrics index templates, dashboard validation, and ITSI readiness across VMware, vCenter Logs, ESXi Logs, Extractions, Indexes, Metrics, and Metrics Indexes |
| `splunk-database-ta-setup` | Database Supported Add-ons | Render, install, and validate package-verified Splunk Supported Add-ons for Microsoft SQL Server, MySQL, and Oracle Database, including DB Connect handoffs, SQL Server file/perfmon inputs, install commands, and validation searches |
| `splunk-netapp-ontap-ta-setup` | NetApp ONTAP Supported Add-ons | Render, install, and validate package-verified NetApp ONTAP add-ons, including scheduler/worker placement, ONTAP indexes, `ontap:*` and Hydra source types, troubleshooting checks, and ITSI storage handoffs |
...
| `splunk-kvstore-admin` | Splunk App Key Value Store (KV Store) | Render and validate KV Store backup, restore, storage-engine migration, SHC member resync, and `kvstore-status` health for standalone search heads or search head clusters |
| `splunk-cim-data-model` | CIM data models (`Splunk_SA_CIM`) | Render and validate CIM data model acceleration overrides (`datamodels.conf`), rebuild/backfill helpers, and CIM population/compliance audits for ES and ITSI readiness |
| `splunk-knowledge-objects` | Knowledge-object governance | Inventory, audit, and govern saved searches, lookups, macros, eventtypes, tags, and field extractions, including sharing/ownership metadata and ownership reassignment |
| `splunk-ingest-actions` | Splunk Ingest Actions | Render and validate Ingest Actions RFS S3/filesystem destinations (`outputs.conf`), ruleset specs for filter/mask/set-index/route, a manual heavy-forwarder preview, and ruleset status |
| `splunk-ddaa-archive` | Splunk Cloud Dynamic Data Active Archive (DDAA) | Render and validate DDAA archive retention via ACS (`searchableDays` + `splunkArchivalRetentionDays`), enable/update payloads, archive/restore audits, and the UI restore handoff |
| `splunk-secure-gateway` | Splunk Secure Gateway / Mobile | Render and validate Spacebridge connectivity, `splunk_secure_gateway` enablement, token-auth readiness, device registration handoff, and an MDM AppConfig template for Connected Experiences |
| `splunk-dashboard-studio` | Splunk Platform Dashboard Studio | Render and validate `version=2` dashboard JSON definitions plus the `data/ui/views` source XML from SPL panels, and publish dashboards via REST |
| `splunk-kvstore-admin-setup` | Splunk App Key Value Store | Back up, restore, clean, migrate (WiredTiger), and upgrade the KV Store, manage maintenance mode, and govern collections/lookup definitions for standalone or SHC |
| `splunk-cim-data-model-setup` | `Splunk_SA_CIM` + `datamodels.conf` | Accelerate CIM data models, constrain `cim_<model>_indexes`, and map sourcetypes into CIM with eventtypes/tags, with tstats validation (distinct from ES-context acceleration) |
| `splunk-knowledge-objects-setup` | Knowledge object governance | Render and apply saved searches, macros, CSV/KV Store lookups, eventtypes, tags, and field knowledge with sharing/ownership (ACL) governance |
| `splunk-ingest-actions-setup` | Splunk Ingest Actions | Render and apply ingest-time rulesets (filter/mask/drop/route) as props/transforms plus RFS `[rfs:]` S3 destinations |
| `splunk-ddaa-archive-setup` | Splunk Cloud Dynamic Data Active Archive | Set per-index `splunkArchivalRetentionDays` via ACS, with restore and disable runbooks for the Splunk Web-only operations |
| `splunk-secure-gateway-setup` | `splunk_secure_gateway` | Enable/disable Secure Gateway, validate Spacebridge egress, and render deployment, Private Spacebridge, MDM, and device-registration runbooks |
| `splunk-dashboard-studio-setup` | Splunk Platform Dashboard Studio | Build version 2 dashboard JSON + `data/ui/views` XML and create/update views via REST with ACL governance (distinct from the Observability dashboard builder) |
...
| `galileo-mcp-server-setup` | Official Galileo MCP Server (`https://api.galileo.ai/mcp/http/mcp`) | Render, validate, probe, and deep-audit safe Model Context Protocol client setup for Cursor, VS Code, Codex, Claude Code, and AWS Kiro with secret-file guardrails, live tool-schema drift checks, product-coverage gap audits, setup/validate scripts, and handoffs to Galileo platform, Agent Control, Splunk Platform HEC, and Splunk Observability workflows |
| `galileo-platform-setup` | Galileo SaaS/Enterprise platform -> Splunk Platform and Splunk Observability Cloud | Render, validate, and optionally apply Galileo readiness, object lifecycle provisioning for projects/log streams/datasets/prompts/experiments/metrics, Agent Observability Controls console and control-span handoffs, full Galileo feature coverage handoffs for REST API/custom deployments, SSO/RBAC, Luna-2, scorers, workflows, multimodal observability for image/audio/PDF-document traces, multimodal quality metrics, tags/metadata, enterprise retention/TTL/privacy, Agent Graph and console debugging, MCP tool calls, cookbooks, async jobs, and release compatibility, `export_records` to Splunk HEC with metadata-only media validation, Observe OpenTelemetry/OpenInference snippets, legacy Protect invoke snippets, Evaluate annotation/feedback handoffs, Splunk HEC/OTLP/OTel Collector handoffs, and Observability dashboard/detector handoffs; supports `--o11y-only` to omit Splunk Platform HEC dependencies |
...
| `splunk-observability-azure-integration` | Azure -> Splunk Observability Cloud | Standalone reusable: render, apply, validate, discover, and diagnose the Splunk Observability Cloud Azure integration end-to-end (polling + Terraform only; Event Hub streaming is not in the wire contract). REST `type=Azure`; Service Principal `tenantId`+`appId`+`secretKey` via file-only delivery (appId/secretKey are redacted on GET — drift detection uses SHA-256 hash sidecar); `azureEnvironment` `AZURE` or `AZURE_US_GOVERNMENT`; per-subscription subscription list; ~80-entry services enum with `additionalServices` escape hatch; `customNamespacesPerService`; `resourceFilterRules` tag-based filtering; `syncGuestOsNamespaces`; `useBatchApi`; `importAzureMonitor`; `pollRate` 60–600 s (wire milliseconds); `namedToken` ForceNew warning; Terraform `signalfx_azure_integration` pinned to `splunk-terraform/signalfx ~> 9.0`; Azure CLI SP creation + Monitoring Reader + Reader role assignment scripts; Bicep role-assignment template (opt-in); multi-subscription management-group-scoped role assignment; conflict matrix (`services` non-empty, poll-rate bounds, namedToken ForceNew, GovCloud realm guard); cross-skill handoffs to `splunk-app-install` (Splunkbase 3110 logs + 4882 dashboards), `splunk-observability-otel-collector-setup` (AKS host telemetry), `splunk-observability-dashboard-builder`, and `splunk-observability-native-ops`. |
| `splunk-observability-gcp-integration` | GCP -> Splunk Observability Cloud | Standalone reusable: render, apply, validate, discover, and diagnose the Splunk Observability Cloud GCP integration end-to-end. REST `type=GCP`; supports `SERVICE_ACCOUNT_KEY` and file-backed `WORKLOAD_IDENTITY_FEDERATION`. WIF requires Splunk's official generated mode-600 `gcp_wif_config.json`, delivered as compact/stringified `workloadIdentityFederationConfig`; it never fabricates realm principals or pool/provider fields. Payloads include `projects.syncMode`; `projectKey` remains write-only with SHA-256 drift tracking. Terraform and gcloud handoffs are limited to verified SA-key behavior; GCP logs, GKE, dashboards, and detector handoffs remain available. |
| `splunk-observability-aws-lambda-apm-setup` | AWS Lambda -> Splunk Observability Cloud APM | Render, validate, and optionally apply Splunk OpenTelemetry Lambda layer (`signalfx/splunk-otel-lambda`, beta, publisher `254067382080`) APM instrumentation for AWS Lambda functions. Covers Node.js 18/20/22, Python 3.9–3.13, Java 8/11/17/21 on x86_64 and arm64; per-runtime `AWS_LAMBDA_EXEC_WRAPPER` wiring; secret-safe `SPLUNK_ACCESS_TOKEN` delivery via Secrets Manager or SSM SecureString (resolve references; token never in files or argv); layer ARN baked snapshot with opt-in live refresh; vendor-coexistence detection (Datadog / New Relic / AppDynamics / Dynatrace / ADOT); X-Ray coexistence flag; GovCloud/China refusal; IAM egress stub; AWS CLI / Terraform / CloudFormation variants; rollback; discover-functions; doctor; handoffs to `splunk-observability-aws-integration`, `splunk-observability-dashboard-builder`, `splunk-observability-native-ops`, and `splunk-connect-for-otlp-setup`. |
...
| `splunk-observability-synthetics-setup` | Splunk Synthetic Monitoring | Render focused browser/API/HTTP/SSL/port Synthetic Monitoring plans, native-ops compatible specs, run-now and waterfall artifact handoffs, and dashboard or detector follow-ups |
| `splunk-observability-slo-setup` | Splunk Observability SLOs | Render focused SLO intake, SLI source decisions, SLO payload intents, `/slo/validate` handoffs, and deep-native workflow specs |
| `splunk-observability-metrics-pipeline-setup` | Splunk Metrics Pipeline Management | Render focused MPM usage, cardinality, route/drop/archive/aggregate, exception-review, and deep-native workflow handoff assets |
...
| `splunk-cloud-acs-admin-setup` | Splunk Cloud Admin Config Service (ACS) administration | Render, inventory, preflight, safely apply, and audit ACS allowlists, indexes, HEC tokens, users, roles, capabilities, app permissions, private connectivity, outbound ports, DDSS self-storage, limits, maintenance windows, and restarts |
| `splunk-cloud-acs-allowlist-setup` | Compatibility alias for ACS allowlists | Legacy allowlist-only workflow; new work should use `splunk-cloud-acs-admin-setup` |
