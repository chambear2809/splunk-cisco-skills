...
| `cisco-collaboration-setup` | Shared baseline; Python 3 and Bash; optional `PyYAML` is supported but not required. | No live access or credentials. Requires only a reviewed strict v1 spec and local sanitized evidence files for enabled CDR/CMR, CIM, or partner-package evidence paths; all collection and device work remains a handoff. |
| `cisco-data-fabric-setup` | Shared baseline; `PyYAML` for YAML intake specs. | Splunk deployment/version/cloud/region and per-surface entitlement context; optional reviewed child specs or tenant URLs for delegated renders. Machine Data Lake, Catalog, Cloud Control, and AI Canvas remain entitlement/UI handoffs where no stable public CRUD contract exists. |
| `cisco-defenseclaw-deskside-setup` | `ssh`, `curl`, `jq`, and a pre-pinned SSH host key; remote Lemonade, Python 3.10+, and network access to Cisco's signed GitHub releases. | Authorized Linux deskside user; Lemonade model registration plus explicit approval to replace an active model; the script never accepts or stores credentials. |
...
| `galileo-lemonade-instrumentation-setup` | Python 3 with PyYAML; an installed Splunk OTel Collector for runtime validation; OpenAI/OpenInference packages only for caller-side instrumentation. | Exact Galileo instance/API endpoint, project and Log stream IDs, owner-only Galileo API-key runtime file, healthy Lemonade/Splunk baseline, and explicit telemetry source/content policy. |
| `galileo-mcp-server-setup` | `node` for the dependency-free stdio bridge; `codex` when registering Codex; Python for live MCP probes. | Galileo API key material in owner-only files or client secret stores; target MCP clients (Cursor, VS Code, Codex, Claude Code, AWS Kiro). Render and probe phases avoid inline secrets. |
| `galileo-on-prem-kubernetes-setup` | Bash and Python 3 with PyYAML for the non-mutating deployment router and coverage doctor. | Exact Galileo instance console URL, environment classification, versioned deployment intake, and child-skill ownership decisions; no credentials are needed for offline render and coverage. |
| `galileo-on-prem-stack-setup` | Bash, Python 3 with PyYAML, Helm, and kubectl for offline rendering and connected read-only preflight; no bootstrap or mutation tooling is invoked. | Exact kube context with read-only discovery access; pinned local Galileo Stack and galileoctl chart archives and hashes; questionnaire values; safe references to owner-only repository-token and secret-values files; explicit storage, DNS, TLS, routing, node-pool, backup, approval, retention, and Galileo/CSE joint-session handoff evidence. The skill never accepts live mutation authority. |
| `galileo-on-prem-agent-control-setup` | Bash, Python 3 with PyYAML, Helm, and kubectl. | Pinned packaged Agent Control chart/archive, exact Stack ownership contract, dedicated database and migration readiness, non-secret values, owner-only secret-values file, routing/TLS, approval, and backup evidence. |
| `galileo-on-prem-luna-studio-setup` | Bash, Python 3 with PyYAML, Helm, and kubectl; GPU tooling only for live in-cluster training validation. | Pinned Luna Studio chart/archive, exact Stack ownership contract, dedicated database and object storage, four native Secret references supplied out of band, training topology, routing/TLS, approval, and backup evidence. |
| `galileo-on-prem-air-gap-setup` | Bash and Python 3; approved read-only registry inspection and scanner tools for structural verification. | CSE-provided chart/image/CLI/model manifest, read-only source and destination digest evidence, target architectures, approved internal registry, and egress policy. Registry writes remain a Galileo/CSE operator handoff and are never performed by the skill. |
| `galileo-platform-setup` | Shared baseline; `curl` for healthchecks; `galileo>=2.2.0` for experiment groups; optional `galileo-protect`; Python 3 for the generic-alert HEC relay; operator-owned HTTPS reverse proxy for public webhook ingress. | Galileo API key file and lifecycle details; a distinct mode-0600 Galileo webhook shared-token file plus Splunk HEC token file for the alert relay; optional Splunk Observability token/realm; enterprise enablement and LLM integration for AI Assistant; console access for global dashboards and other UI-only handoffs. |
| `lemonade-splunk-otel` | Python 3 with PyYAML; Debian APT/systemd tooling on the target; installed collector binary for exact config validation. | SSH/root access to the Lemonade host, discovered service/config/package paths, owner-only Splunk credential files, approved upgrade/rollback window, and backend read access for validation. |
...
| `splunk-observability-coding-agent-instrumentation-setup` | Shared baseline Python only. | No live access required; parent router emits child skill orchestration plans only. |
| `splunk-observability-codex-instrumentation-setup` | Python 3.11+ for `tomllib`; `codex` CLI for post-render `--strict-config` checks and runtime use. | Splunk Observability realm; runtime `SPLUNK_ACCESS_TOKEN` environment placeholder for direct mode, or local/external collector endpoints. |
| `splunk-observability-claude-code-instrumentation-setup` | Python 3.11+; `claude` CLI for runtime use; Splunk Distribution of OTel Collector for local-collector mode. | Splunk Observability realm and `SPLUNK_O11Y_TOKEN_FILE` for direct mode; local/external OTel Collector endpoint; Galileo project name and `GALILEO_API_KEY_FILE` when Galileo fan-out is enabled. |
...
| `splunk-observability-database-monitoring-setup` | Python 3.10+; Docker or Podman for pinned Collector config validation; Kubernetes apply: `kubectl`, Helm, Mike Farah `yq` v4; Linux apply: `/usr/bin/otelcol` pinned to Splunk OTel Collector 0.155.0, Mike Farah `yq` v4, systemd, root or approved noninteractive sudo; Python `fcntl` support provides portable transaction locking; Windows validation handoff: PowerShell and pinned Collector 0.155.0. | DBMon entitlement and supported realm; production engine/platform/version; explicit collector CPU/memory plus non-secret sizing evidence; one scrape owner, reviewed Kubernetes context, secure URL-form SQL Server/Oracle datasource or reviewed self-managed transport exception, and target validation identities; tenant API token in an owner-only file; installed chart token Secret or private host environment; existing DB credential Secrets/private env file; explicit upgrade/reconfiguration gates; database-owner approval for grants/restarts; application-owner approval for optional APM propagation. MySQL 5.7+ and MariaDB 10.5+ are supported, with explain-plan gaps recorded for MySQL 5.7 and MariaDB. |
...
| `splunk-kvstore-admin` (**Deprecated** -> `splunk-kvstore-admin-setup`) | Help-only compatibility alias. | No independent operational requirements; read the canonical replacement and use its current safety gates. |
| `splunk-cim-data-model` (**Deprecated** -> `splunk-cim-data-model-setup`) | Help-only compatibility alias. | No independent operational requirements; read the canonical replacement and use its current safety gates. |
| `splunk-knowledge-objects` (**Deprecated** -> `splunk-knowledge-objects-setup`) | Help-only compatibility alias. | No independent operational requirements; read the canonical replacement and use its current safety gates. |
| `splunk-ingest-actions` (**Deprecated** -> `splunk-ingest-actions-setup`) | Help-only compatibility alias. | No independent operational requirements; read the canonical replacement and use its current safety gates. |
| `splunk-ddaa-archive` (**Deprecated** -> `splunk-ddaa-archive-setup`) | Help-only compatibility alias. | No independent operational requirements; read the canonical replacement and use its current safety gates. |
| `splunk-secure-gateway` (**Deprecated** -> `splunk-secure-gateway-setup`) | Help-only compatibility alias. | No independent operational requirements; read the canonical replacement and use its current safety gates. |
| `splunk-dashboard-studio` (**Deprecated** -> `splunk-dashboard-studio-setup`) | Help-only compatibility alias. | No independent operational requirements; read the canonical replacement and use its current safety gates. |
...
| `splunk-ingest-actions-setup` | Shared baseline. | Target-app conf write permission for `configs/conf-props` and `configs/conf-transforms` (eval/mask/drop), plus `configs/conf-outputs` for route-s3 RFS destination staging; `edit_ingest_rulesets` or Splunk Web access separately for status and the route-rule handoff; S3 bucket and access/secret key files for RFS staging; topology-appropriate deploy step. |
...
| `splunk-secure-gateway-setup` | Shared baseline; `nc` or `curl` only for the Enterprise egress check. | Explicit Splunk Enterprise REST access for app enable/disable and outbound 443 to Spacebridge; Splunk Web/MDM operator access for the rendered skeleton/runbooks (no live configure API). Splunk Cloud is plain-render/support only with no local probe or live REST. |
