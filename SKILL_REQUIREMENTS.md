...
| `galileo-platform-setup` | Shared baseline; `curl` for healthcheck/readiness probes; `galileo` Python SDK for object lifecycle apply; optional `galileo-protect` for Protect stage creation. | Galileo API key file, project/log-stream/dataset/prompt/experiment/metric/Protect stage details; optional Splunk HEC token file, Splunk Observability token file, and target realm for HEC export, OTel Collector, dashboards, and detectors. |
