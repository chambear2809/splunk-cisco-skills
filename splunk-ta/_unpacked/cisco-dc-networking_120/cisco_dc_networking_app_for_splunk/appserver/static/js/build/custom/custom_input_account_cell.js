...
class CustomInputCell {
    ...
    render() {
        ...
        if ((!this.row.nexus_9k_account || this.row.nexus_9k_account.trim() === "") && (!this.row.nd_account || this.row.nd_account.trim() === "") && (!this.row.apic_account || this.row.apic_account.trim() === "")) {
            html = TEMPLATE;

        } else {
            if (this.row.nexus_9k_account !== undefined || this.row.nd_account !== undefined || this.row.apic_account !== undefined) {
                let accountName = this.row.nexus_9k_account || this.row.nd_account || this.row.apic_account;
                let maxLength = 50; 
