...
| `splunk-observability-gcp-integration` | GCP -> Splunk Observability Cloud | Render, apply, validate, discover, and diagnose the Splunk Observability Cloud GCP integration (REST `type=GCP`). Supports `SERVICE_ACCOUNT_KEY` or file-backed `WORKLOAD_IDENTITY_FEDERATION`; WIF requires Splunk's official generated mode-600 `gcp_wif_config.json`, sent as compact/stringified `workloadIdentityFederationConfig`, with no fabricated realm principal/pool/provider mapping. Emits `projects.syncMode`; treats `projectKey` as write-only; provides hash drift detection, service controls, SA-only Terraform/gcloud handoffs, and GCP logs/GKE handoffs. |
